...
const createMockFlags = (issueId: string): Flag[] => [
  {
    ...
    user: { id: mockUsers[1].id, username: mockUsers[1].username, name: mockUsers[1].name, email: mockUsers[1].email, role: mockUsers[1].role, createdAt: mockUsers[1].createdAt }
  },
];
...
const mockService = {
  ...
  flagIssue: async (id: string, reason: string) => {
    ...
    const flag: Flag = {
      ...
      createdAt: new Date(),
      updatedAt: new Date(),
      user: { id: mockUsers[0].id, username: mockUsers[0].username, name: mockUsers[0].name, email: mockUsers[0].email, role: mockUsers[0].role, createdAt: mockUsers[0].createdAt }
    };
  ...
  getNearbyIssues: async (latitude: number, longitude: number, radius: number = 5) => {
    ...
    const nearby = mockIssues.filter(issue => {
      const ilat = issue.location.latitude;
      const ilon = issue.location.longitude;
      const dist = haversine(latitude, longitude, ilat, ilon);
      return dist <= radius;
    });
    
    // Sort by reportedAt descending (most recent first)
    nearby.sort((a, b) => (b.reportedAt as any) - (a.reportedAt as any));
    return nearby;
  }
};

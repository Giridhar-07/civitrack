...
const AdminDashboard: React.FC = () => {
  ...
  const fetchUsers = async (pageIndex = usersPage, pageSize = usersRowsPerPage) => {
    try {
      const usersResp = await userService.adminGetUsers({ page: pageIndex + 1, limit: pageSize });
      if (usersResp && Array.isArray(usersResp.users)) {
        setUsers(usersResp.users);
        setUsersPagination(usersResp.pagination || null);
      } else {
        setUsers([]);
  ...
  useEffect(() => {
    // Load users whenever pagination changes
    fetchUsers();
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Initial users fetch
      try {
        const usersResp = await userService.adminGetUsers({ page: 1, limit: 20 });
        if (usersResp && Array.isArray(usersResp.users)) {
          setUsers(usersResp.users);
          setUsersPagination(usersResp.pagination || null);
        } else {
  ...
  // Handlers for Users pagination
  const handleUsersChangePage = (event: unknown, newPage: number) => {
    setUsersPage(newPage);
  };

  const handleUsersChangeRowsPerPage = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleEditUserSubmit = async () => {
    ...
    try {
      await userService.updateUser(userToEdit.id, editUserFormData);
  ...
  const updateStats = (updatedIssues: Issue[]) => {
    ...
    const flagged = updatedIssues.filter(issue => issue.flags && issue.flags.length > 0 && issue.flags.some(f => !f.resolved)).length;
  ...
  useEffect(() => {
    if (issues && issues.length > 0) {
      // Filter flagged issues (only unresolved flags)
      const flagged = issues.filter(issue => 
        issue.flags && 
        issue.flags.length > 0 && 
        issue.flags.some(flag => !flag.resolved)
      );
  ...
  }, [issues, users, usersPagination, statusRequestsData]);
  ...
  const handleResolveFlag = async (flagId: string, issueId: string) => {
    try {
      await issueService.resolveFlag(flagId);
      
      // Refetch issues to get updated flag status
      await fetchIssues();
      
      showSuccess('Flag resolved successfully');
    } catch (error) {
    ...
    <Box sx={{ width: '100%' }}>
              ...
              {paginatedIssues.length > 0 ? (
                paginatedIssues.map((issue) => (
                  <TableRow key={issue.id}>
                    ...
                    <TableCell>{issue.user?.username || issue.user?.name || issue.reportedBy}</TableCell>
              ...
              <Table sx={{ minWidth: 650 }} aria-label="flagged issues table">
                ...
                <TableBody>
                  {flaggedIssues.map((issue) => {
                    const unresolvedFlag = issue.flags?.find(flag => !flag.resolved);
                    return (
                      <TableRow key={issue.id}>
                        <TableCell component="th" scope="row">
                          {issue.title}
                        ...
                        <TableCell>
                          {unresolvedFlag?.reason ?? 'No reason provided'}
                        </TableCell>
                        <TableCell>
                          {unresolvedFlag
                            ? (unresolvedFlag.user?.name || unresolvedFlag.user?.username || 'Unknown')
                            : 'Unknown'}
                        </TableCell>
                        <TableCell>
                          {unresolvedFlag 
                            ? formatDate(unresolvedFlag.createdAt as any)
                            : 'Unknown'}
                        </TableCell>
                        <TableCell align="right">
                          <IconButton 
                          ...
                          <IconButton 
                            ...
                            onClick={() => unresolvedFlag && handleResolveFlag(unresolvedFlag.id as any, issue.id)}
                            size="small"
                            color="success"
                          >
      ...
      <TabPanel value={tabValue} index={3}>
        ...
        <TablePagination
          rowsPerPageOptions={[5, 10, 25]}
          component="div"
          count={usersPagination?.total ?? users.length}
      ...
      {/* User Edit Dialog */}
      <Dialog open={userEditDialogOpen} onClose={handleUserEditDialogClose} maxWidth="sm" fullWidth>
        <DialogTitle>
          Edit User
          <IconButton
            aria-label="close"
        ...
        <DialogActions>
          <Button onClick={handleUserEditDialogClose}>Cancel</Button>
          <Button 
      ...
      {/* User Delete Dialog */}
      <Dialog open={userDeleteDialogOpen} onClose={handleUserDeleteDialogClose} maxWidth="xs" fullWidth>
        <DialogTitle>
          Confirm Delete User
          <IconButton
            aria-label="close"
            onClick={handleUserDeleteDialogClose}
            sx={{ position: 'absolute', right: 8, top: 8 }}
          >
            <CloseIcon />
          </IconButton>

...
export const getStatusRequestsValidation = [
  query('page')
    .optional()
    .isInt({ min: 1 }).withMessage('Page must be a positive integer'),
  query('limit')
  ...
  query('status')
    .optional()
    .isIn(['pending', 'approved', 'rejected']).withMessage('Invalid request status')
];

export const reviewStatusRequestValidation = [
  param('id')
    .isUUID(4).withMessage('Invalid status request ID'),
  body('action')
...
export const getIssueStatusRequestsValidation = [
  param('id')
    .isUUID(4).withMessage('Invalid issue ID')
];
...
export const aiChatValidation = [
  ...
  body('history')
    .optional()
    .isArray({ max: 20 }).withMessage('History must be an array with at most 20 items'),
  body('history.*.role')
    .optional()
  ...
  body('history.*.content')
    ...
    .isLength({ min: 1, max: 2000 }).withMessage('History content must be between 1 and 2000 characters')
];

export const aiQuickHelpValidation = [
  query('topic')
    .optional()
